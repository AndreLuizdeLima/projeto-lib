import styles from './ViewsAutorLivro.module.css'
import { Link, useParams } from "react-router-dom"
import { useGetItems } from '../../../hooks/useGetItems'
import { useEffect, useState } from 'react'
import { useUpdateItem } from '../../../hooks/usePutItems'

const ViewAutorLivro = () => {

    const { id } = useParams()
    const { documents: autor, loading, error } = useGetItems('AUTOR', id);
    const [autorNome, setautorNome] = useState('')
    const [newNome, setNewNome] = useState('')
    const { update, loading: loading_put, error: error_put } = useUpdateItem("AUTOR");


    useEffect(() => {
        if (autor && autor.length > 0) {
            setautorNome(autor[0].nome);
        }
    }, [autor])
